...
- finalTip / negotiationScript / negotiationScriptVariants (MANDATORY PRICE-DIRECTION LOGIC): any specific target price you mention MUST be LOWER than offeredPrice (${offeredPrice} ${currency}) — never equal to or higher than it. The entire point of negotiating is paying LESS than the price on offer, so a target above offeredPrice is a contradiction and is never valid, regardless of where offeredPrice sits relative to marketFairPriceMin/Max.
  - If verdict is "good" (offeredPrice is already at or below marketFairPriceMin): there is nothing to negotiate down to — do NOT suggest a negotiation target price at all. Instead finalTip should say this is already a strong price and advise closing the deal quickly while verifying authenticity/condition/warranty; negotiationScript should be a brief message confirming interest and asking to finalize, not a price-reduction request.
  - If verdict is "fair" or "bad": a negotiation target price is appropriate, but it must sit strictly below offeredPrice (e.g. anchored near marketFairPriceMin or a reasonable point between marketFairPriceMin and offeredPrice) — never above offeredPrice and never above marketFairPriceMax.
